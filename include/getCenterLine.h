...
//namespace CGAL{
//inline Gmpfr sqrt( const Gmpfr& x ) {
//    return x.sqrt();
//}
//}
struct PairHash{
...
// 确保<a, b> = <b, a>
inline std::pair<int, int> make_mono_pair(int a, int b) { return a < b ? std::make_pair(a, b) : std::make_pair(b, a); }

const double eps = 1. / (1 << 20), eps2 = eps * eps, eps_limit = 0.25 / (1ll << 62);
...
struct CenterLineSolver{
	...
	// results
	std::vector<Segment_2> res_segments, aug_segments;
	...
	static Vector_2 calc_real_speed(const Vector_2 &src_vector, const Vector_2 &dest_vector){
		Vector_2 src = src_vector / CGAL::sqrt(src_vector.squared_length()), dest = dest_vector / CGAL::sqrt(dest_vector.squared_length());
		...
		if(inner_product(src_vector, dest_vector) > 0){
			...
			v_bisector = dest - src;
			if(outer_product(src_vector, v_bisector) < 0) v_bisector = -v_bisector;
		}
	...
	static Vector_2 calc_speed(const Vector_2 &src_vector, const Vector_2 &dest_vector){
		...
		if(inner_product(src_vector, dest_vector) > 0){
			...
			Line_2 dest_norm(CGAL::ORIGIN, dest_vector.perpendicular(CGAL::LEFT_TURN));
			//v_bisector = v_norm + dest.perpendicular(CGAL::LEFT_TURN);
	...
	struct PointData {
		...
		PointData(const Point_2 &_start_loc, const FT &_start_time, const Vector_2 &src, const Vector_2 &dest) : start_loc(_start_loc), start_time(_start_time), end_time(0), src_vector(src), dest_vector(dest), branches{0}, prev(this), next(this) { }
		Vector_2 speed() const {
			// TODO: 处理极端情况(夹角接近+-180°)
			return calc_speed(src_vector, dest_vector);
		}
		Ray_2 path() const {
			//std::cout << "Vector = "<< src_vector << " " << dest_vector << std::endl;
			//std::cout << "loc = "<< start_loc << std::endl;
	...
	struct EdgeData {
		...
		Segment_2 new_loc(FT new_time){
			return Segment_2(src_point->new_loc(new_time), dest_point->new_loc(new_time));
		}
	...
	struct Interval {
		...
		Interval() : is_inf(false), is_empty(true), is_endpoint(0) {}
		...
		Interval operator + (const Interval &b) const {
			...
			if(begin < b.begin) ans.begin = b.begin, ans.begin_point = b.begin_point;
			else ans.begin = begin, ans.begin_point = begin_point;

			if(is_inf && b.is_inf) ans.is_inf = true;
			else if(is_inf) {
				//if(b.end <= ans.begin)
				if(!less(ans.begin, b.end)) ans.is_empty = true;
				else ans.end = b.end, ans.end_point = b.end_point;
			}
			else if(b.is_inf){
				//if(end <= ans.begin)
				if(!less(ans.begin, end)) ans.is_empty = true;
				else ans.end = end, ans.end_point = end_point;
			}
			else{
				//if(end <= ans.begin || b.end <= ans.begin)
				if(!less(ans.begin, end) || !less(ans.begin, b.end)) ans.is_empty = true;
				else if(end < b.end) ans.end = end, ans.end_point = end_point;
				else ans.end = b.end, ans.end_point = b.end_point;
			}
			//if(ans.is_inf || ans.begin < ans.end)
			if(!ans.is_inf && !less(ans.begin, ans.end)) ans.is_empty = true;
			return ans;
		}
	};
	...
	struct Event{
		...
		EdgeData *a, *b;
		EventType type;
		Event(FT t, EdgeData *e0, EdgeData *e1, EventType tp) : time(t), a(e0), b(e1), type(tp) { assert(e0); assert(e1); }
		bool is_active() const {
	...
	bool calc_interval_on_line(const Line_2 &bisector, const EdgeData &edge, Interval &interval, bool parallel = false){
		...
		if(parallel) {
			if(!a || !b) return false;
			if(tmp_inner_product < 0) std::swap(a, b);
			if(!(tmp_p = boost::get<Point_2>(&*a)) || !(tmp_q = boost::get<Point_2>(&*b))) return false;
			std::cout << "tmp_p, tmp_q" << *tmp_p << " " << *tmp_q << std::endl;
			FT loc_p = calc_loc_on_line(bisector, *tmp_p), loc_q = calc_loc_on_line(bisector, *tmp_q);
			std::cout << loc_p << " " << loc_q << std::endl;
			//if(loc_p >= loc_q)
			if(!less(loc_p, loc_q)) return false;
			...
			if(equal(src_loc, dest_loc)) return false; // 已经退化为一个点
			if(!less(origin_loc, src_loc) && !less(origin_loc, dest_loc)) return false;
			...
			if(!x){
				...
				else if((tmp_p = boost::get<Point_2>(&*a))){ begin = *tmp_p; }

				std::cout << begin << std::endl;

				FT loc_p = calc_loc_on_line(bisector, begin), loc_q;
				
				if(!b) interval.set(loc_p, begin, is_endpoint);
				...
				else assert(false); // 线段和中线没有交点，end不可能在中线上
				return true;
			}
			// 线段一开始就与中线相交
			// 需要排除一开始从中线远离的情况
				...
				if(equal(src_loc, origin_loc)) is_endpoint = 1;
				//else if(dest_loc == origin_loc)
				else if(equal(dest_loc, origin_loc)) is_endpoint = 2;
				...
				if(!a && !b){
					...
					if(is_endpoint == 1){
						...
						else if(less(0, outer_product(src->speed(), bisector.to_vector()), FT(eps) * calc_line_scale(bisector))){
							return false;
						}
						...
						else assert(false); // dest不可能也在中线上
						...
						else if(less(outer_product(dest->speed(), bisector.to_vector()), 0, FT(eps) * calc_line_scale(bisector))){
							return false;
						}
						...
						else assert(false); // src 不可能也在中线上
						interval.set(loc_p, loc_q, begin, end, is_endpoint);
					}
					else {
						// 此时交点一定在内部
						...
						std::cout << bisector << std::endl << *tmp_src << " " << *tmp_dest << std::endl;
			...
			else assert(false); // bisector 与segment不平行
		}
		return true;
	}
	...
	void append_collision_events(EdgeData *edge){
		...
		Line_2 bisector, edge_opposite_line = line_data_pool[edge->corr_line].opposite();
		int e_it_id = 0;
		for(auto e_it : edge_data_pool){
			++e_it_id;
			if(e_it == edge) break; // 只处理edge_data_pool中当前edge与位于之前的边相交的事件
			if(!e_it->is_active()) continue;
			// 相邻边不需要添加事件
			if(e_it->src_point == edge->dest_point || edge->src_point == e_it->dest_point) continue;
			const Line_2 &cur_line = line_data_pool[e_it->corr_line];
			Vector_2 it_vector = cur_line.to_vector(), edge_vector = edge_opposite_line.to_vector();
			...
			if(parallel &&
				(tmp_inner_product < 0 && less(0, tmp_inner_product * tmp_inner_product, absolute_eps))) continue;
			std::cout << "e_it_id=" << e_it_id-1 << std::endl;
			...
			if(bisector_not_found) {
				if(tmp_inner_product >= 0) bisector = CGAL::bisector(cur_line, edge_opposite_line);
				else {
					auto intersect = CGAL::intersection(cur_line, edge_opposite_line);
					Point_2 *origin;
					if(!intersect || !(origin = boost::get<Point_2>(&*intersect))) continue;
					bisector = CGAL::bisector(cur_line.perpendicular(*origin), edge_opposite_line.opposite().perpendicular(*origin));
					//std::cout << *origin << std::endl;
					//std::cout << cur_line.perpendicular(*origin) << std::endl;
					//std::cout << edge_opposite_line.opposite().perpendicular(*origin) << std::endl;
				}
				if(inner_product(bisector.to_vector(), cur_line.to_vector().perpendicular(CGAL::LEFT_TURN)) < 0)
					bisector = bisector.opposite();
			}
			else bisector = bisector_map[line_pair];

			// 判断是否确定会相交
			// L{0,1}, R{0,1}: 两个线段的端点; point_l, point_r: 线段公共部分的端点
			// l{0,1}, r{0,1}: 两个线段端点与bisector相交的位置; loc_l, loc_r: 相交部分在bisector上的位置
			...
			std::cout << interval0.to_string() << std::endl << interval1.to_string() << std::endl;
			if(!tmp1 || !tmp2) continue; // (!tmp1 && !tmp2)会漏掉情况
			//if(!calc_interval_on_line(bisector, *e_it, interval0) || !calc_interval_on_line(bisector, *edge, interval1)) continue;
			Interval interval = interval0 + interval1;
			std::cout << interval.to_string();
			if(interval.is_empty) continue;
		
			if(bisector_not_found) bisector_map[line_pair] = bisector;
			...
			else events.emplace(time, e_it, edge, EventType::NOT_PARALLEL);
		}
	}
	bool is_debugging;
	...
	void append_line_data(const Polygon_2 &poly, size_t &line_it){
		...
		for(int i = 0;i < len;++i){
			...
			//PointData(const Point_2 &_start_loc, const FT &_start_time, const Vector_2 &src, const Vector_2 &dest);
			PointData *new_point = new PointData(poly.edge(i).target(), FT(0), src_vector, dest_vector);
			src_vector = dest_vector;
			tmp_point.push_back(new_point);
			//EdgeData(int line, PointData<K> *src, PointData<K> *dest) : corr_line(line), src_point(src), dest_point(dest), prev(this), next(this) {}
		...
		for(int i = 0;i < len;++i){
			src_edge->next = tmp_edge[i]; tmp_edge[i]->prev = src_edge;
			src_edge = tmp_edge[i];
			edge_data_pool.push_back(src_edge);
			src_point->next = tmp_point[i]; tmp_point[i]->prev = src_point;
	...
	std::vector<PointData*> &point_pool, std::vector<EdgeData*> &edge_pool){
		...
		if(is_parallel(base_v, edge_v)){
			...
			if(type == 0){
				inter_seg = new PointData(location, this->cur_time, base->dest_point->src_vector, edge->src_point->dest_vector);
				...
				else edge->src_point->branches[1] = inter_seg;

				auto inter0 = CGAL::intersection(bisector, base->src_point->path()), inter1 = CGAL::intersection(bisector, edge->dest_point->path());
				assert(inter0 && inter1);
				Point_2 *p0 = boost::get<Point_2>(&*inter0), *p1 = boost::get<Point_2>(&*inter1);
				assert(p0 && p1);
				FT l0 = calc_loc_on_line(bisector, *p0), l1 = calc_loc_on_line(bisector, *p1);
				std::cout << "p0="<< *p0 << std::endl;
				std::cout << "p1="<< *p1 << std::endl;
				if(less(l0, l1)){
					inter_seg->end_time = this->cur_time; inter_seg->end_loc = *p1;
					if(edge->dest_point->end_time == FT(0)) CutHalfEdge(base, edge->next, 0, *p1, point_pool, edge_pool);
					// TODO: 是否有可能循环回来?
					inter_seg->branches[0] = edge->dest_point->branches[0];
				}
				else if(less(l1, l0)) {
					inter_seg->end_time = this->cur_time; inter_seg->end_loc = *p0;
					if(base->src_point->end_time == FT(0)) CutHalfEdge(edge, base->prev, 1, *p0, point_pool, edge_pool);
					inter_seg->branches[0] = base->src_point->branches[0];
				}
				else{
					inter_seg->end_time = this->cur_time; inter_seg->end_loc = *p0;
					if(base->src_point->end_time == FT(0)) CutHalfPoint(base->prev, edge->next, *p0, point_pool, edge_pool);
					inter_seg->branches[0] = base->src_point->branches[0];
				}
			}
			else{ // type == 1
				inter_seg = new PointData(location, this->cur_time, edge->dest_point->src_vector, base->src_point->dest_vector);
				point_pool.push_back(inter_seg);
				if(!edge->src_point->branches[0]){
					edge->src_point->end_time = this->cur_time;
					edge->src_point->end_loc = location;
				...
				auto inter0 = CGAL::intersection(bisector, base->dest_point->path()), inter1 = CGAL::intersection(bisector, edge->src_point->path());
				assert(inter0 && inter1);
				Point_2 *p0 = boost::get<Point_2>(&*inter0), *p1 = boost::get<Point_2>(&*inter1);
				assert(p0 && p1);
				FT r0 = calc_loc_on_line(bisector, *p0), r1 = calc_loc_on_line(bisector, *p1);
				if(less(r0, r1)){
					inter_seg->end_time = this->cur_time; inter_seg->end_loc = *p0;
					if(base->dest_point->end_time == FT(0)) CutHalfEdge(edge, base->next, 0, *p0, point_pool, edge_pool);
					inter_seg->branches[0] = base->dest_point->branches[0];
				}
				else if(less(r1, r0)) {
					inter_seg->end_time = this->cur_time; inter_seg->end_loc = *p1;
					if(edge->src_point->end_time == FT(0)) CutHalfEdge(base, edge->prev, 1, *p1, point_pool, edge_pool);
					inter_seg->branches[0] = edge->src_point->branches[0];
				}
				else{
					inter_seg->end_time = this->cur_time; inter_seg->end_loc = *p1;
					if(edge->src_point->end_time == FT(0)) CutHalfPoint(edge->prev, base->next, *p1, point_pool, edge_pool);
					inter_seg->branches[0] = edge->src_point->branches[0];
				}
			}
			return;
		...
		if(type == 0){
			PointData *point = new PointData(location, this->cur_time,
				base->src_point->dest_vector, edge->src_point->dest_vector);
			...
			else edge->src_point->branches[1] = point;
			...
			edge_pool.push_back(src); edge_pool.push_back(dest);
			src->prev = base->prev; src->prev->next = src;
			dest->next = edge->next; dest->next->prev = dest;
			src->next = dest; dest->prev = src;
		}
		else {
			PointData *point = new PointData(location, this->cur_time,
				edge->src_point->dest_vector, base->src_point->dest_vector);
			point_pool.push_back(point);
			...
			else edge->dest_point->branches[1] = point;
			...
			edge_pool.push_back(src); edge_pool.push_back(dest);
			src->prev = edge->prev; src->prev->next = src;
			dest->next = base->next; dest->next->prev = dest;
			src->next = dest; dest->prev = src;
		}
	}
	...
	std::vector<PointData*> &point_pool, std::vector<EdgeData*> &edge_pool){
		...
		Line_2 prev_line = line_data_pool[prev->corr_line], next_line = line_data_pool[next->corr_line];
		Vector_2 prev_v = prev_line.to_vector(), next_v = next_line.to_vector();
		PointData *point = new PointData(location, this->cur_time, prev->dest_point->src_vector, next->src_point->dest_vector);
		point_pool.push_back(point);
		...
		else prev->dest_point->branches[1] = point;
		...
		else next->src_point->branches[1] = point;
		...
		if(is_parallel(prev_v, next_v) && inner_product(prev_v, next_v) < 0){
			...
			auto inter0 = CGAL::intersection(bisector, prev->src_point->path()), inter1 = CGAL::intersection(bisector, next->dest_point->path());
			assert(inter0 && inter1);
			Point_2 *p0 = boost::get<Point_2>(&*inter0), *p1 = boost::get<Point_2>(&*inter1);
			assert(p0 && p1);
			FT r0 = calc_loc_on_line(bisector, *p0), r1 = calc_loc_on_line(bisector, *p1);
			if(less(r0, r1)){
				point->end_time = this->cur_time; point->end_loc = *p0;
				if(prev->src_point->end_time == FT(0)) CutHalfEdge(next, prev->prev, 1, *p0, point_pool, edge_pool);
				point->branches[0] = prev->src_point->branches[0];
			}
			else if(less(r1, r0)){
				point->end_time = this->cur_time; point->end_loc = *p1;
				if(next->dest_point->end_time == FT(0)) CutHalfEdge(prev, next->next, 0, *p1, point_pool, edge_pool);
				point->branches[0] = next->dest_point->branches[0];
			}
			else{
				point->end_time = this->cur_time; point->end_loc = *p1;
				if(next->dest_point->end_time == FT(0)) CutHalfPoint(prev->prev, next->next, *p1, point_pool, edge_pool);
				point->branches[0] = next->dest_point->branches[0];
			}
			return;
		}
		...
		new_prev->prev = prev->prev; new_next->next = next->next;
		new_prev->next = new_next; new_next->prev = new_prev;
		edge_pool.push_back(new_prev); edge_pool.push_back(new_next);
	}
	...
	void operator () (const Poly_with_holes& polygon){
		...
		for(auto it = polygon.holes_begin(); it != polygon.holes_end();++it) append_line_data(*it, line_it);

		// for each pair of non-adjacent edges, calculate the bisector of them and when the
		// endpoints of both edges intersect with the bisector. store the edge pair indexed with the
		// start time of the overlapping interval if it exists.
		//				-> as a function: "for a new edge with each edge in the Iterable list"
		// TODO: 预处理速度，减少求速度的消耗

		// set an event for each start time of the overlapping interval of an edge pair.
		while(!events.empty()) events.pop();
		cur_time = 0;
		for(auto e_it : edge_data_pool) append_collision_events(e_it);
		...
		//		(相遇位置位于某个线段内部: 把这个线段拆成两半，然后让点终止，分裂成两个点分别与两边的edge片段相，重建多边形)
		// > case point vs point: modify the linking relation of points and edges;
		//		(相遇位置为两个线段的顶点: 终止两个顶点轨迹，重建新的顶点，重新连接多边形)
		...
		while(!events.empty()) {
			...
			for(size_t i = 0;i < line_data_pool.size();++i){
				line_loc[i] = Line_2(line_data_pool[i].point(0) + line_speed[i] * exact_time, line_data_pool[i].to_vector());
			}
			std::cout << "time = sqrt(" << event.time << ") = " << exact_time << std::endl;
			EdgeData *event_a = event.a, *event_b = event.b;
			Segment_2 seg0 = event_a->new_loc(line_loc), seg1 = event_b->new_loc(line_loc);
			Line_2 bisector = bisector_map[std::make_pair(event_a->corr_line, event_b->corr_line)];
			FT l0 = calc_loc_on_line(bisector, seg0.source()), r0 = calc_loc_on_line(bisector, seg0.target());
			FT l1 = calc_loc_on_line(bisector, seg1.target()), r1 = calc_loc_on_line(bisector, seg1.source());
			std::cout << "event.a=" << event_a->corr_line << "     event.b=" << event_b->corr_line << std::endl;
			std::cout << event_a << " " << event_b << std::endl;
			std::cout << "l0 r0 l1 r1=" << l0 << " " << r0 << std::endl << l1 << " " << r1 << std::endl << std::endl;
			std::cout << "seg0 seg1=" << seg0 << " " << seg1 << std::endl;
			if(event.type == EventType::PARALLEL){
				/* 输出公共部分 TODO: 改成创建点 */
				this->res_segments.emplace_back((l0 < l1) ? seg1.target() : seg0.source(),
					(r0 < r1) ? seg0.target() : seg1.source());
				if(event_a->prev != event_b->next){
					//if(l0 < l1)
					if(less(l0, l1)) CutHalfEdge(event_a, event_b->next, 0, seg1.target(), point_pool, edge_pool);
					//else if(l0 > l1)
					else if(less(l1, l0)) CutHalfEdge(event_b, event_a->prev, 1, seg0.source(), point_pool, edge_pool);
					else{
				...
				if(event_a->next != event_b->prev){
					//if(r0 < r1)
					if(less(r0, r1)) CutHalfEdge(event_b, event_a->next, 0, seg0.target(), point_pool, edge_pool);
					//else if(r0 > r1)
					else if(less(r1, r0)) CutHalfEdge(event_a, event_b->prev, 1, seg1.source(), point_pool, edge_pool);
					else{
				...
				std::cout << "parallel: a=" << event_a->src_point->start_loc << " " << event_a->src_point->end_loc << std::endl;
				std::cout << "parallel: a=" << event_a->dest_point->start_loc << " " << event_a->dest_point->end_loc << std::endl;
				std::cout << "parallel: b=" << event_b->src_point->start_loc << " " << event_b->src_point->end_loc << std::endl;
				std::cout << "parallel: b=" << event_b->dest_point->start_loc << " " << event_b->dest_point->end_loc << std::endl;
			}
			else{
				if(l0 > r0){
		...
		for(PointData *it : point_data_pool){
			...
			if(outer_product(it->src_vector, it->dest_vector) >= 0 &&
				tmp_inner_product < 0 && tmp_inner_product * tmp_inner_product > FT(0.25 - eps) * absolute_scale){
				this->res_segments.emplace_back(it->start_loc, it->end_loc);
			}
			else this->aug_segments.emplace_back(it->start_loc, it->end_loc);
		}
	}
	CenterLineSolver(){
		is_debugging = false;
...
/// get the CenterLine and save in point, segments, PolyParts.
template<typename K, class Poly_with_holes, class Poly>
static void getCenterLine(const Poly_with_holes& polygon, std::vector<CGAL::Point_2<K>>& point, std::vector<std::pair<int, int>>& segments,
	std::vector<CGAL::Point_2<K>> &aug_point, std::vector<std::pair<int, int>> &aug_segments, std::vector<Poly>& PolyParts) {
	using FT = typename K::FT;
	using Polygon_2 = CGAL::Polygon_2<K>;
	using Polygon_with_holes_2 = CGAL::Polygon_with_holes_2<K>;
	using Point_2 = CGAL::Point_2<K>;
	...
	point.clear(); segments.clear(); PolyParts.clear();

	Polygon_2 outer = polygon.outer_boundary();
	std::vector<Polygon_2> holes(polygon.holes_begin(), polygon.holes_end());
	Vector_2 polygon_offset(outer.left_vertex()->x(), outer.bottom_vertex()->y());
	...
	for(auto &poly : holes) for(auto i = poly.vertices_begin();i != poly.vertices_end();++i) {
		std::cout << "p=" << (*i - polygon_offset) << std::endl;
		poly.set(i, *i - polygon_offset);
	}

	//std::vector<Polygon_2> PolyParts;
	//CGAL::optimal_convex_partition_2(poly.vertices_begin(), poly.vertices_end(), std::back_inserter(PolyParts));
	//std::vector<Poly> PolyParts;
	//std::vector<std::vector<size_t>> origin_edge;
	//CGAL::drawPartition(PolyParts);
	...
	CenterLineSolver<K, Poly_with_holes, Poly> solver;
	solver(Poly_with_holes(outer, holes.begin(), holes.end()));

	for(auto &seg : solver.res_segments){
		point.push_back(seg.source());
		point.push_back(seg.target());
	}
	std::sort(point.begin(), point.end());
	auto unique_end = std::unique(point.begin(), point.end());
	point.erase(unique_end, point.end());
	for(auto &seg : solver.res_segments){
		int a = std::lower_bound(point.begin(), point.end(), seg.source()) - point.begin();
		int b = std::lower_bound(point.begin(), point.end(), seg.target()) - point.begin();
		segments.emplace_back(a, b);
	}
	for(auto &p : point) p += polygon_offset;

	for(auto &seg : solver.aug_segments){
		aug_point.push_back(seg.source());
		aug_point.push_back(seg.target());
	}
	std::sort(aug_point.begin(), aug_point.end());
	unique_end = std::unique(aug_point.begin(), aug_point.end());
	aug_point.erase(unique_end, aug_point.end());
	for(auto &seg : solver.aug_segments){
		int a = std::lower_bound(aug_point.begin(), aug_point.end(), seg.source()) - aug_point.begin();
		int b = std::lower_bound(aug_point.begin(), aug_point.end(), seg.target()) - aug_point.begin();
		aug_segments.emplace_back(a, b);
	}
	for(auto &p : aug_point) p += polygon_offset;
}
